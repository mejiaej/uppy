...
class Uploader {
  ...
  uploadMultipart () {
    const file = fs.createReadStream(this.path)
    logger.debug(JSON.stringify(file), 'upload.multipart.readStream')
    // upload progress
    let bytesUploaded = 0
    file.on('data', (data) => {
    ...
    file.on('end', function () {
      logger.debug(JSON.stringify(file), 'upload.multipart.readStream.end')
    })

    file.on('error', function(err) {
      logger.debug(JSON.stringify(err), 'upload.multipart.readStream.error')
      logger.debug(JSON.stringify(file), 'upload.multipart.readStream.error.file')
    });
    ...
    if (this.options.useFormData) {
      ...
      logger.info(JSON.stringify(reqOptions), 'upload.multipart.request.formData');
